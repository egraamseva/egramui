...
import { Plus, Edit, Trash2, Eye, EyeOff, ArrowUp, ArrowDown, Copy, GripVertical, Search, Filter } from 'lucide-react';
...
import { platformLandingPageApi } from '../../routes/api';
...
function SectionItem({
  ...
  onEdit: (section: PlatformSection) => void;
  ...
  const getSectionTypeColor = (type: string) => {
    const colors: Record<string, string> = {
      ...
      'CTA': 'bg-indigo-100 text-indigo-800',
    };
...
export function PlatformLandingPageManager() {
  const [sections, setSections] = useState<PlatformSection[]>([]);
  const [filteredSections, setFilteredSections] = useState<PlatformSection[]>([]);
  ...
  const fetchSections = async () => {
    try {
      setLoading(true);
      const data = await platformLandingPageApi.getSections();
  ...
  const handleEdit = (section: PlatformSection) => {
    setEditingSection(section);
    setIsEditorOpen(true);
  };

  const handleSave = async (sectionData: Partial<PlatformSection>): Promise<PlatformSection> => {
    try {
      let savedSection: PlatformSection;
      if (editingSection) {
        savedSection = await platformLandingPageApi.updateSection(editingSection.id, sectionData);
      } else {
        savedSection = await platformLandingPageApi.createSection(sectionData as any);
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await platformLandingPageApi.deleteSection(id);
  ...
  const handleDuplicate = async (id: string) => {
    try {
      ...
      const duplicatedData: Partial<PlatformSection> = {
      ...
      await platformLandingPageApi.createSection(duplicatedData as any);
      toast.success('Section duplicated successfully');
      await fetchSections();
    } catch (error: any) {
      toast.error('Failed to duplicate section: ' + (error.message || 'Unknown error'));
    }
  };
  ...
  const handleToggleVisibility = async (id: string) => {
    try {
      const section = sections.find(s => s.id === id);
      if (section) {
        await platformLandingPageApi.toggleVisibility(id, section.isVisible);
  ...
  const handleMoveUp = async (id: string) => {
    ...
    try {
      await platformLandingPageApi.updateOrder(newSections[index].id, index);
      await platformLandingPageApi.updateOrder(newSections[index - 1].id, index - 1);
  ...
  const handleDrop = async (e: React.DragEvent, dropIndex: number) => {
    ...
    try {
      for (let i = 0; i < newSections.length; i++) {
        await platformLandingPageApi.updateOrder(newSections[i].id, i);
      }
      toast.success('Order updated successfully');
      await fetchSections();
    } catch (error: any) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold">Platform Landing Page</h2>
          <p className="text-sm sm:text-base text-muted-foreground">Manage sections for the main platform landing page</p>
        </div>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              Create your first section to start building the platform landing page
            </p>
